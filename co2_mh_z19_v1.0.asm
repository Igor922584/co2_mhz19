...
RESET:
    ...
    ldi    temp, 0b00000111        ; Порт B работает на выход PB0->MOSI, PB1->RCLK (защелка)
    out    DDRB, temp              ; PB2->SRCLK (тактовый сигнал), PB4+PB3-> работает на вход
	
    ldi    temp, 0b00000000        ; Порт работает на выход, все выходы при включении
    out    PortB, temp             ; ставим в 0, подт. резист. PB4+PB3 не подключаем (есть в схеме)
...
TWI_Pause:
    ldi     temp, 100                ; Поменять занчения для корректировки паузы
TWI_Pause_Loop:
    subi    temp, 1
