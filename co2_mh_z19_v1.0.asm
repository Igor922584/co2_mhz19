...
; 9.3. Проверить правильность установки значений флагов этапа выполнения программы (ЕСТЬ ОШИБКИ).
...
; 10. Написать код для получения данных по TWI от датчика LPS331AP.
...
rx_uart_mhz:       .byte  9        ; Регистры для хранения полученных байтов от MH-Z19
...
Main:
    rcall    Pause1                ; Проверка необходимости Увеличения значения индикатора
...
N_Mask:
.db 0x3f,0x06,0x5b,0x4f,0x66,0x6d,0x7d,0x07,0x7f,0x6f,0xbf,0x86,0xdb,0xcf,0xe6,0xed,0xfd,0x87,0xff,0xef
...
URXC:
    ...
    lds     temp, count_rx_uart      ; Прибавляем к адресу начала переменно значение счетчика 
    ...
    breq    Stop_rx             ; Если 9, то запрещаем прерывания на передачу, восст. знач. счетчика
...
Flag:
    ...
    sbrc    temp, 3                 ; Пропустить след команду, если настройку таймера Т1 выполнять не требуется
...
End_Pause_Temp_Conv:
    ...
    ldi     temp, 0b00001000        ; Устанавливаем флаг в позицию начала инициализации 2
