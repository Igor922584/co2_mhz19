...
; 10. Написать код для получения данных по TWI от датчика LPS331AP (аналогично работе UART и MH-Z19.
; 10.1. Написать п/п для передачи/получения данных по TWI.
; 10.1.1. Программная реализация TWI.
; 10.1.2. Тактовый сигнал SCL со скважностью 2, высокий/низкий уровни по 1,5 мкс.
; 10.1.3. П/п: старт, стоп, повстарт, запись, чтение.
; 10.1.4. Объмен данными с датчиком LPS331AP после окончания обмена данными по UART с MH-Z19.  
; 10.2. Написать п/п для организации обмена данными по TWI с датчиком LPS331AP.
; 10.3. Скорректировать обработку флагов (запрос/получение данных от LPS331AP в период ожидания
;       окончания преобразования значения температуры датчиком DS18B20).
; 11. Добавить код для обеспечения индикации контроля работы датчиков.
...
coont_rx_twi:      .byte  1        ; Счетчик полученных байт по TWI
count_tx_twi:      .byte  1        ; Счетчик отправленных байт по TWI
press_twi:         .bate  3        ; Регистры для хранения значения давления от LPS331AP
...
;============================================================================================================
;Команды для чтения значения давления из датчика LPS331AP
LPS331AP_read:
.db 0xB8,0x28,0xB9
...
Stop_rx:
    ...
    ldi     temp, 0b00000010        ; Устанавливаем флаг в позицию этапа сохранение значения концентрации
    sts     flag_ex_stage, temp     ; СО2 (Calc_co2)
...
;======================================================================================================
; Полуяение значения атмосферного давления от датчика LPS331AP по TWI, программная реализация протокола
Atm_Press:
...
;+++Добавить команду SAD+W (если 9 бит, то добавить регистр в ОЗУ с количеством передаваемых бит)
    rcall   TWI_Write
    rcall   Sak_TWI

;+++Добавить команду запроса адреса ячейки памяти SUB
    rcall   TWI_Write
    rcall   Sak_TWI
    rcall   Rep_Start_TWI

;+++Добавить команду SAD+R
    rcall   TWI_Write
    rcall   Sak_TWI
    rcall   TWI_Read
    rcall   Mak_TWI

;+++Добавить п/п сохранения 1-го принятого байта, линию SDA держать в "0" без тактовых импульсов
    rcall   TWI_Read
    rcall   Mak_TWI

;+++Добавить сохранение второго принятого байта
    rcall   TWI_Read
    rcall   Mak_TWI
    rcall   NMak_TWI          ; Скорее всего 2 последних п/п будут не нужны, последовательное 
    rcall   Stop_TWI          ; последовательное выполнение команд для завершения приема,
                              ; сохранение принятых данных и преобразование в BCD
...
; Проверка ответа датчика по TWI
Sak_TWI:
...
TWI_Write:   ; Бит на выход PB5, затем поднимаем SCL, держим линию 1.5 мкс, опускаем SCL 
;    ldi     tx_rx_count, 8      ; Количество передаваемых по USI бит
		
; Correct SPI_Transfer_Loop:
...
End_Atm_Press:
    ldi     temp, 0b00001000        ; Устанавливаем флаг в позицию запроса значения атмосферного давления
...
Flag:
    ...
    sbrc    temp, 1                 ; Если флаг установлен, переход к п/п сохранению и расчету значения
    rjmp    Calc_co2                ; концентрации СО2 в ОЗУ
...
;==========================================================================================================
;Настрпойка таймера Т1 для чтения байта от датчика DS18B20
T1_Read_Ds18b20:
...
; Запуск передачи данных по UART к датчику MH-Z19
    ...
    ldi     temp, 0b00000100      ; Устанавливаем флаг в позицию ожидания окончания преобразования 
...
Calc_co2:
    ...
    rjmp    Main                    ; До проверки оконч. времени для преобразования t вывод на LED
